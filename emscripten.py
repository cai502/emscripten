...
def function_tables_and_exports(funcs, metadata, mem_init, glue, forwarded_data, settings, outfile, DEBUG):
    ...
    pre += include_objc_metadata(metadata, settings)
    ...
    export_external_symbol_names(outfile.name, settings)
    ...
    if settings['RELOCATABLE']:
      global_funcs += ['g$' + extern for extern in metadata['externs'] + settings['EXPORTED_VARIABLES']]
...
def memory_and_global_initializers(pre, metadata, mem_init, settings):
  ...
  staticbump = metadata['staticBump'] + 512
...
def get_all_implemented(forwarded_json, metadata):
  return set(metadata['implementedFunctions'] + forwarded_json['Functions']['implementedFunctions'].keys()) # XXX perf?
...
def include_objc_metadata(metadata, settings):

  # objc
  ret = "var EMSCRIPTEN_OBJC_METADATA = {\n"
...
def export_external_symbol_names(outfile_name, settings):
...
def create_asm_setup(debug_tables, function_table_data, metadata, settings):
  ...
  if settings['RELOCATABLE']:
    ...
    def check(extern):
      ...
      return ''
    for extern in metadata['externs'] + settings['EXPORTED_VARIABLES']:
