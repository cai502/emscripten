...
var TOMBO_S_IFDIR = {{{ cDefine('S_IFDIR') }}};
var TOMBO_S_IFLNK = {{{ cDefine('S_IFLNK') }}};
var TOMBO_MAP_PRIVATE = {{{ cDefine('MAP_PRIVATE') }}};
...
/***/ function(module, exports) {
	...
	module.exports = {
	  ...
	  node_ops: {
	    ...
	    lookup: function lookup(parent, name, ex) {
	      if (ex) return;
	  ...
	  stream_ops: {
	    ...
	    mmap: function mmap(stream, buffer, offset, length, position, prot, flags) {
	      ...
	      if (!(flags & TOMBO_MAP_PRIVATE) && (contents.buffer === buffer || contents.buffer === buffer.buffer)) {
...
/******/ ]);
      // replace TOMBOFS
      delete this.mount;
      for (var key in tombofs) {
        if (tombofs.hasOwnProperty(key)) {
          this[key] = tombofs[key];
        }
      }
