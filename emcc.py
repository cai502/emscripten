  ...
  try:
    with ToolchainProfiler.profile_block('parse arguments and setup'):
      ...
      for i in range(len(newargs)):
        if newargs[i] in ['-l', '-L', '-I', '-F']:
          # Scan for individual -l/-L/-I/-F arguments and concatenate the next arg on if there is no suffix
      ...
      framework_dirs = [shared.path_from_root('system', 'frameworks')]
      for i in range(len(newargs)): # find input files XXX this a simple heuristic. we should really analyze based on a full understanding of gcc params,
        ...
        if not arg.startswith('-'):
          ...
          # don't remove, it should be passed to not only linker but also compiler
        elif arg.startswith('-l'):
          libs.append((i, arg[2:]))
          newargs[i] = ''
      ...
      for framework in options.frameworks:
        logging.debug('looking for framework "%s"', framework)
        found = False
        for framework_dir in framework_dirs:
          path = os.path.join(framework_dir, framework + ".framework", framework)
          if os.path.exists(path):
            logging.debug('found framework "%s" at %s', framework, path)
      ...
      if filename_type_suffix(target) not in JS_CONTAINING_SUFFIXES or options.ignore_dynamic_linking:
        def check(input_file):
          if filename_type_ending(input_file) in DYNAMICLIB_ENDINGS:
            ...
            return False
          elif not filename_type_ending(input_file) in SOURCE_ENDINGS + BITCODE_ENDINGS + STATICLIB_ENDINGS:
            return False
          else:
            return True
      ...
      if shared.Settings.BINARYEN:
        ...
        if shared.Settings.BINARYEN_ASYNC_COMPILATION == 1 and 'interpret' not in shared.Settings.BINARYEN_METHOD and 'asmjs' not in shared.Settings.BINARYEN_METHOD and 'interpret-asm2wasm' not in shared.Settings.BINARYEN_METHOD and not shared.Settings.RUNNING_JS_OPTS:
...
def parse_args(newargs):
  ...
  for i in range(len(newargs)):
    ...
    if newargs[i].startswith('-O'):
      ...
      newargs[i] = ''
      newargs[i+1] = ''
    elif newargs[i] == '-framework':
      options.frameworks.append(newargs[i+1])
      newargs[i] = ''
      newargs[i+1] = ''
