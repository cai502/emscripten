...
def run():
  ...
  parser.add_option('--profile', dest='profile', default='',
    help='Specifies browser profile. (supports firefox only)')

  opts_with_param = ['--browser', '--timeout_returncode', '--timeout', '--silence_timeout', '--log_stderr', '--log_stdout', '--hostname', '--port', '--serve_root', '--profile']
  ...
  if options.android:
    ...
    if not options.no_browser or options.browser_info:
      ...
      if 'safari' in browser_exe.lower():
        ...
        browser_args = ['--incognito', '--enable-nacl', '--enable-pnacl', '--disable-restore-session-state', '--enable-webgl', '--no-default-browser-check', '--no-first-run', '--allow-file-access-from-files', '--user-data-dir=/tmp/hogefuga', '--js-flags=--stack_trace_limit=10000']
    #    if options.no_server:
    #      browser_args += ['--disable-web-security']
      elif 'firefox' in browser_exe.lower():
        processname_killed_atexit = 'firefox'
        if options.profile:
          browser_args = ['-P', options.profile]
